...
impl McpResource for TendConfigsResource {
    fn uri(&self) -> &str { "tend://configs" }
    fn name(&self) -> &str { "Tend Configs" }
    fn description(&self) -> &str { "List all discovered .tend.json configs" }
    ...
    fn read(&self, _ctx: &ToolContext) -> Result<Value, ToolFailure> {
        ...
        match tend::discover::discover_configs(&root, None) {
            Ok(nodes) => {
                let configs: Vec<Value> = nodes.iter().map(|n| {
                    json!({
...
impl McpResource for TendRunsResource {
    fn uri(&self) -> &str { "tend://runs" }
    fn name(&self) -> &str { "Tend Runs" }
    fn description(&self) -> &str { "Recent tend run summaries" }
    fn mime_type(&self) -> &str { "application/json" }
    fn read(&self, _ctx: &ToolContext) -> Result<Value, ToolFailure> {
