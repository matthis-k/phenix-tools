...
impl McpPrompt for StitchPlanSyncPrompt {
    fn name(&self) -> &str { "stitch/plan-sync" }
    fn description(&self) -> &str { "Guide to planning a multi-repo sync" }
    fn arguments(&self) -> Vec<PromptArgument> { vec![] }
    fn get(&self, _args: HashMap<String, Value>, _ctx: &ToolContext) -> Result<Value, ToolFailure> {
        Ok(json!({"messages": [{"role": "user", "content": "I need to plan a sync across repos."}]}))
    }
}
...
impl McpPrompt for StitchSplitFeatureCommitsPrompt {
    fn name(&self) -> &str { "stitch/split-feature-commits" }
    fn description(&self) -> &str { "Guide to splitting changes into coherent feature commits" }
    fn arguments(&self) -> Vec<PromptArgument> { vec![] }
    fn get(&self, _args: HashMap<String, Value>, _ctx: &ToolContext) -> Result<Value, ToolFailure> {
        Ok(json!({"messages": [{"role": "user", "content": "Help me split changes into coherent feature commits."}]}))
    }
}
