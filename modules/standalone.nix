{ lib, ... }: {
  perSystem = { config, pkgs, ... }: let
    ...
    mkCargoCheck = name: description: cargoArgs: extraNativeBuildInputs:
  ...
  in {
    packages = {
      ...
      check = pkgs.writeShellApplication {
        ...
        runtimeInputs = [ pkgs.cargo pkgs.rustc pkgs.rustfmt pkgs.clippy pkgs.git tendCliPkg ];
        text = ''
          set -euo pipefail
          echo "=== cargo fmt ==="
          cargo fmt --all --check
    ...
    checks = {
      cargo-check = mkCargoCheck
        "phenix-tools-cargo-check"
        "cargo check --workspace --all-targets"
        "cargo check --workspace --all-targets"
        [ pkgs.cargo pkgs.rustc ];

      cargo-test = mkCargoCheck
        "phenix-tools-cargo-test"
        "cargo test --workspace"
        "cargo test --workspace"
        [ pkgs.cargo pkgs.rustc pkgs.git ];

      cargo-fmt = mkCargoCheck
        "phenix-tools-cargo-fmt"
        "cargo fmt --all --check"
        "cargo fmt --all --check"
        [ pkgs.cargo pkgs.rustfmt ];

      cargo-clippy = mkCargoCheck
        "phenix-tools-cargo-clippy"
        "cargo clippy --quiet --workspace --all-targets -- -D warnings"
        "cargo clippy --quiet --workspace --all-targets -- -D warnings"
        [ pkgs.cargo pkgs.clippy pkgs.rustc ];

      tend-gate = pkgs.runCommand "phenix-tools-tend-gate" {
        nativeBuildInputs = [ tendCliPkg pkgs.git pkgs.cargo pkgs.rustc pkgs.rustfmt pkgs.clippy pkgs.stdenv.cc ];
        inherit cargoDeps;
        src = filteredSrc;
      } ''
        export HOME=$TMPDIR/home
